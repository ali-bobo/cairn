...
### 段 1 — HTML 報告強化（原待辦 B；2026-07-08 對照 html.rs 現況修正範圍）

**問題（修正版）**：`Finding.evidence` 明細**已經有** `<details>` 折疊展示
（`html.rs:414-433`，gate-redesign 時一併做了）——原待辦 B 的「無法展開看 evidence」
主張過時。真正還缺的是**導航**：
1. 無法依 severity / artifact / 標題關鍵字做 client-side 篩選
2. 無法跨 Finding 看同一 binary 出現在哪幾處（同 binary 聚合）
3. findings/observations 數量大時無排序切換

**解法（範圍已縮）**：
1. severity / artifact / 關鍵字 client-side 篩選（inline JS，零外部資源）
2. 「同 binary 出現次數」摘要欄或聚合區塊
3. 順手修 ir-panels 兩個殘留：顯示 `WtsSession.state_active`、修「對外連線」面板
   標題語意（見殘留風險登記表）
...
| **live proc collector 不採集 cmdline/integrity（恆 None）**→ parentchild 的 encoded-PS(+40)/LOLBAS-args(+30)/unsigned-high-integrity(+15) 訊號在 live 掃描永不觸發 | 段8審計 F-1（高） | `cairn-collectors-win/src/proc.rs:142-143`；修法=NtQueryInformationProcess 讀 PEB cmdline + OpenProcessToken 讀 integrity RID（新增 unsafe 需審查）。**live 偵測有效性第一優先缺口** |
| 掃描無進度回饋/逾時，verbose 掃描下使用者無法分辨「還在跑」與「卡死」 | 段8審計 F-8（高） | launcher 只印一行「執行掃描中」；修法=per-collector 開始/完成列印（cairn 已有 tracing，launcher 繼承 stderr 即可見） |
