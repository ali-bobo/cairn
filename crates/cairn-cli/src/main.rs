...
#[derive(Parser)]
#[command(name = "cairn", version, about = "Authorized Windows live-forensics triage engine")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum Cmd {
    ...
    Evtx { files: Vec<PathBuf>, #[arg(long)] rules: Option<PathBuf> },
    /// Fetch + pin the Sigma ruleset (S4).
    UpdateRules { #[arg(long)] pin: Option<String> },
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        Cmd::Evtx { files, rules } => {
            tracing::info!(?files, ?rules, "TODO T4-T7: parse EVTX, run Sigma, write timeline");
